...
func Benchmark_loop_Convert_Filtered(b *testing.B) {
	integers := range_.Of(0, max)
	for i := 0; i < b.N; i++ {
		loop.Convert(loop.Filter(integers, even).Next, strconv.Itoa).ForEach(func(element string) {
			resultStr = element
		})
	}
}
...
func Benchmark_loop_Convert_Filtered_rengefunc(b *testing.B) {
	integers := range_.Of(0, max)
	for i := 0; i < b.N; i++ {
		for element := range loop.Convert(loop.Filter(integers, even).Next, strconv.Itoa).All {
			resultStr = element
		}
	}
}
