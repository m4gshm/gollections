...
In the example is used only small set of slice functions as link:#slicefilter[slice.Filter], link:#sliceconv[slice.Conv] link:./slice/api.go#L166[Convert], and link:#slicereduce[Reduce]. More you can look in the link:./slice/api.go[slice] package.
...
This is a shorter version of the previous example that used short aliases link:#sumof[sum.Of] and link:#filterandonvert[filter.AndConvert].
