...
Gollections is set of functions for link:#slices[slices], link:#maps[maps], link:#seq-seq2-seqe[iter.Seq, iter.Seq2] and additional implementations of link:#loop-kvloop-and-breakable-versions-breakloop-breakkvloop[iterator] and data structures such as link:#mutable-collections[ordered map] or link:#mutable-collections[set] aimed to reduce boilerplate code.
...
*Be careful* when use several slice functions subsequently like [source,go]`slice.Filter(slice.Convert(...))`. This can lead to unnecessary RAM consumption. Consider link:#seq[seq] instead of slice API.
...
[#loop]
=== Deprecated: link:./loop/api.go[loop], link:./kv/loop/api.go[kv/loop] and breakable versions link:./break/loop/api.go[break/loop], link:./break/kv/loop/api.go[break/kv/loop]
