...
Gollections is set of functions for link:./slice/api.go[slices], link:./map_/api.go[maps] and additional implementations of data structures such as link:./collection/collection/mutable/omap/api.go[ordered map] or link:./collection/collection/mutable/oset/api.go[set] aimed to reduce boilerplate code.
...
In the example is used only small set of slice functions as link:./slice/api.go#L421[Filter], link:./slice/api.go#L178[Conv] link:./slice/api.go#L166[Convert], and link:./slice/api.go#L529[Reduce]. More you can look in the link:./slice/api.go[slice] package.
...
This is a shorter version of the previous example that used short aliases link:./slice/sum/api.go#L10[sum.Of] and link:./slice/filter/api.go#L9[filter.AndConvert].
...
* creating - link:#slice.Of[slice.Of], range_.Of, clone.Of, slice.DeepClone
* sorting - sort.Asc, sort.Desc, sort.By, asc.Of, desc.Of
* stable sorting and sorting cloned slice
* converting, filtering, summarizing - convert.AndReduce, convert.AndConvert, convert.AndFilter, filter.AndConvert, filter.ConvertFilter, sum.Of, first.Of, last.Of
* versions of aboves with possible result errors - conv.AndReduce
* to map transforming - slice.ToMap, slice.ToMapResolv, group.Of, group.ByMultiple, group.ByMultipleKeys, group.ByMultipleValues
...
[source,go]
----
include::../examples/sliceexamples/slice_examples_test.go[lines=141..143,indent=0]
----
