...
// Track applies the 'consumer' function for every key/value pairs from the 'elements' map. Return the c.ErrBreak to stop
func Track[M ~map[K]V, K comparable, V any](elements M, consumer func(K, V) error) error {
	for key, val := range elements {
		if err := consumer(key, val); err == ErrBreak {
			return nil
		} else if err != nil {
			return err
		}
...
// TrackOrdered applies the 'consumer' function for key/value pairs from the 'elements' map in order of the 'order' slice. Return the c.ErrBreak to stop
func TrackOrdered[M ~map[K]V, K comparable, V any](order []K, elements M, consumer func(K, V) error) error {
	for _, key := range order {
		if err := consumer(key, elements[key]); err == ErrBreak {
			return nil
		} else if err != nil {
			return err
		}
...
// ForKeys applies the 'consumer' function for keys from the 'elements' map . Return the c.ErrBreak to stop.
func ForKeys[M ~map[K]V, K comparable, V any](elements M, consumer func(K) error) error {
	for key := range elements {
		if err := consumer(key); err == ErrBreak {
			return nil
		} else if err != nil {
			return err
		}
...
// ForValues applies the 'consumer' function for values from the 'elements' map . Return the c.ErrBreak to stop..
func ForValues[M ~map[K]V, K comparable, V any](elements M, consumer func(V) error) error {
	for _, val := range elements {
		if err := consumer(val); err == ErrBreak {
			return nil
		} else if err != nil {
			return err
		}
...
// ForOrderedValues applies the 'consumer' function for values from the 'elements' map in order of the 'order' slice. Return the c.ErrBreak to stop..
