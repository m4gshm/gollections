...
func Test_Set_Group_By_Iterator(t *testing.T) {
	groups := iter.Group(set.Of(0, 1, 1, 2, 4, 3, 1, 6, 7).Iter(), func(e int) bool { return e%2 == 0 }).Map()
...
func Test_Set_Convert(t *testing.T) {
	var (
		...
		strings  = loop.Slice(iter.Filter(set.Convert(ints, strconv.Itoa), func(s string) bool { return len(s) == 1 }).Next)
